...
fn array_teste() {
    ...
    println!("Arrays");
      ...
      io::stdin()
          .read_line(&mut index)
          .expect("Falha em ler a linha");

      let index: usize = index
          .trim()
          .parse()
          .expect("Index infoormado não encontrado");

      let elemento = array[index];

      println!("o valor do elemento no index {} é {}", index, elemento);
}
...
fn conceitos_basicos() {
    //---Start---conceitos básicos
        ...
        let dois = tup.2;
            println!("\nvalor X na tupla : {} ", quinhentos);
            println!("\nvalor X na tupla : {} ", seis_quatro);
            println!("\nvalor X na tupla : {} \n", dois);
